...
const resolveFromAddress = () => SMTP_FROM || SMTP_USER || "no-reply@bookin.local";
...
const sendBookingReceiptEmail = async (payload) => {
  ...
  const portalUrl = APP_BASE_URL ? `${APP_BASE_URL.replace(/\/$/, "")}/my-transaction` : "";
  if (!SMTP_HOST) {
    console.info(
  ...
  const tenantText = payload.tenantName?.trim() ? payload.tenantName.trim() : "Tim Tenant";
  ...
  const text = [
    ...
    "",
    portalUrl ? `Lihat detail transaksi di: ${portalUrl}` : "Silakan cek detail transaksi di aplikasi BookIn.",
    "",
    "Hormat kami,",
    "Tim BookIn",
    "",
    ...
    "",
    portalUrl ? `View transaction details: ${portalUrl}` : "Please check your transaction details in the BookIn app.",
    "",
    "Sincerely,",
    "BookIn Team",
    "",
