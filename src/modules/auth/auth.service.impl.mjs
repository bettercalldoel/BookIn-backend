...
class AuthService {
  ...
  registerUser = async (body) => {
    ...
    return {
      message: "Registrasi berhasil. Silakan cek email untuk verifikasi dan membuat password.",
      email: account.email,
      expiresAt
    };
  };
  registerTenant = async (body) => {
    const email = this.normalizeEmail(body.email);
    await this.ensureEmailAvailable(email);
  ...
  registerOrLoginSocial = async (body) => {
    ...
    if (existingOauth) {
      const account2 = await this.prisma.account.findUnique({
        where: { id: existingOauth.accountId },
        include: { userProfile: true, tenantProfile: true }
      });
      if (!account2) throw new ApiError("Akun tidak ditemukan.", 404);
      return this.buildAuthResponse(account2, false);
    }
    const email = this.normalizeEmail(body.email);
    await this.ensureEmailAvailable(email);
    const displayName = body.accountType === AccountType.TENANT ? body.companyName?.trim() || body.name.trim() : body.name.trim();
    const account = await this.prisma.account.create({
      data: {
        email,
        ...
        avatarUrl: body.avatarUrl,
        userProfile: body.accountType === AccountType.USER ? {
          create: { fullName: displayName }
        } : void 0,
        tenantProfile: body.accountType === AccountType.TENANT ? {
          create: { displayName }
        } : void 0
      },
      include: { userProfile: true, tenantProfile: true }
  ...
  loginGoogle = async (body) => {
    ...
    const account = await this.prisma.account.create({
      data: {
        ...
        userProfile: accountType === AccountType.USER ? { create: { fullName: displayName } } : void 0,
        tenantProfile: accountType === AccountType.TENANT ? {
          create: { displayName }
        } : void 0
      },
      include: { userProfile: true, tenantProfile: true }
  ...
  verifyEmail = async (body) => {
    ...
    const passwordHash = nextPassword ? await hashPassword(nextPassword) : void 0;
    ...
    if (!hasPassword && !passwordHash) {
      const { token: resetToken, expiresAt } = await this.createPasswordResetToken(token.accountId);
      ...
      return {
        message: "Email berhasil diverifikasi. Silakan cek email untuk reset password."
      };
    }
    return {
  ...
  updateProfile = async (accountId, body) => {
    if (!accountId) throw new ApiError("Unauthorized.", 401);
    const account = await this.prisma.account.findUnique({
      where: { id: accountId },
      include: { userProfile: true, tenantProfile: true }
  ...
  updatePassword = async (accountId, body) => {
    if (!accountId) throw new ApiError("Unauthorized.", 401);
    const account = await this.prisma.account.findUnique({
      where: { id: accountId }
    });
  ...
  updateEmail = async (accountId, body) => {
    if (!accountId) throw new ApiError("Unauthorized.", 401);
    const account = await this.prisma.account.findUnique({
      where: { id: accountId },
      include: { userProfile: true, tenantProfile: true }
    ...
    const { token, expiresAt } = await this.createEmailVerificationToken(accountId);
    await sendVerificationEmail({
      to: email,
      name: this.resolveAccountName(account),
      token,
    ...
    return {
      message: "Email berhasil diperbarui. Silakan cek email untuk verifikasi ulang.",
      email,
      expiresAt
    };
  };
  buildAuthResponse = (account, isNew) => {
  ...
  async createEmailVerificationToken(accountId) {
    const token = crypto.randomBytes(32).toString("base64url");
    const tokenHash = this.hashToken(token);
    const expiresAt = this.buildTokenExpiry(
      EMAIL_VERIFICATION_TTL_MINUTES,
      MAX_EMAIL_TTL_MINUTES
    );
    await this.saveEmailVerificationToken(accountId, tokenHash, expiresAt);
  ...
  async createPasswordResetToken(accountId) {
    ...
    await this.savePasswordResetToken(accountId, tokenHash, expiresAt);
    return { token, expiresAt };
  }
  buildTokenExpiry(ttlMinutes, maxMinutes) {
    const bounded = Number.isFinite(ttlMinutes) && ttlMinutes > 0 ? Math.min(ttlMinutes, maxMinutes) : maxMinutes;
    return new Date(Date.now() + bounded * 60 * 1e3);
  }
  async saveEmailVerificationToken(accountId, tokenHash, expiresAt) {
